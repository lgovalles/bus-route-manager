...
def test_create_stop(test_client):
    response = test_client.post(
        "/stops/", json={"name": "Test Stop", "latitude": 10.0, "longitude": 20.0}
    )
...
def test_create_route(test_client):
    response = test_client.post(
        "/routes/", json={"name": "Test Route", "code": "TR1", "color": "red", "type": "bus"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_get_route_stops(test_client):
    ...
    # Create stops
    stop1 = test_client.post(
        "/stops/", json={"name": "Stop 1", "latitude": 10.0, "longitude": 20.0}
    ).json()
    stop2 = test_client.post(
        "/stops/", json={"name": "Stop 2", "latitude": 15.0, "longitude": 25.0}
    ).json()
    stop3 = test_client.post(
        "/stops/", json={"name": "Stop 3", "latitude": 20.0, "longitude": 30.0}
    ).json()

    # Create route-stops with specific order
    test_client.post(
        "/route-stops/", json={"route_id": route_id, "stop_id": stop1["id"], "stop_order": 0}
    )
    test_client.post(
        "/route-stops/", json={"route_id": route_id, "stop_id": stop3["id"], "stop_order": 2}
    )
    test_client.post(
        "/route-stops/", json={"route_id": route_id, "stop_id": stop2["id"], "stop_order": 1}
    )
...
def test_update_stop_not_found(test_client):
    response = test_client.put(
        "/stops/999", json={"name": "Updated Stop", "latitude": 15.0, "longitude": 25.0}
    )
    assert response.status_code == 404
    assert response.json()["detail"] == "Stop not found"
...
def test_update_route_not_found(test_client):
    response = test_client.put(
        "/routes/999",
        json={"name": "Updated Route", "code": "UR1", "color": "blue", "type": "train"},
    )
    assert response.status_code == 404
    assert response.json()["detail"] == "Route not found"
...
def test_update_route_stop_not_found(test_client):
    response = test_client.put(
        "/route-stops/999", json={"route_id": 1, "stop_id": 1, "stop_order": 1}
    )
    assert response.status_code == 404
    assert response.json()["detail"] == "RouteStop not found"
...
def test_create_route_stop_invalid_route(test_client):
    response = test_client.post(
        "/route-stops/", json={"route_id": 999, "stop_id": 1, "stop_order": 1}
    )
    assert response.status_code == 404
    assert response.json()["detail"] == "Route not found"
...
def test_create_route_stop_invalid_stop(test_client):
    ...
    response = test_client.post(
        "/route-stops/", json={"route_id": 1, "stop_id": 999, "stop_order": 1}
    )
    assert response.status_code == 404
    assert response.json()["detail"] == "Stop not found"
...
def test_create_stop_invalid_latitude_type(test_client):
    response = test_client.post(
        "/stops/", json={"name": "Test Stop", "latitude": "invalid", "longitude": 20.0}
    )
    assert response.status_code == 422
...
def test_create_stop_invalid_latitude_range(test_client):
    response = test_client.post(
        "/stops/", json={"name": "Test Stop", "latitude": 100.0, "longitude": 20.0}
    )
    assert response.status_code == 422
...
def test_create_stop_invalid_longitude_range(test_client):
    response = test_client.post(
        "/stops/", json={"name": "Test Stop", "latitude": 10.0, "longitude": 200.0}
    )
    assert response.status_code == 422
...
def test_create_route_missing_code(test_client):
    response = test_client.post(
        "/routes/", json={"name": "Test Route", "color": "red", "type": "bus"}
    )
    assert response.status_code == 422
...
def test_create_route_invalid_name_type(test_client):
    response = test_client.post(
        "/routes/", json={"name": 123, "code": "TR1", "color": "red", "type": "bus"}
    )
    assert response.status_code == 422
...
def test_create_route_stop_invalid_stop_order(test_client):
    response = test_client.post(
        "/route-stops/", json={"route_id": 1, "stop_id": 1, "stop_order": -1}
    )
    assert response.status_code == 422
...
def test_wrong_content_type(test_client):
    response = test_client.post(
        "/stops/",
        data="name=Test&latitude=10.0&longitude=20.0",
...
def test_paraderos_data_validation(test_client):
    ...
    stop_data = {"name": "Test Stop", "latitude": 9.9281, "longitude": -84.6783}
...
def test_delete_stop_success(test_client):
    ...
    response = test_client.post(
        "/stops/", json={"name": "To Delete", "latitude": 9.9, "longitude": -84.1}
    )
    assert response.status_code == 200
...
def test_delete_route_stop_success(test_client):
    ...
    stop_id = test_client.post(
        "/stops/", json={"name": "S1", "latitude": 9.9, "longitude": -84.1}
    ).json()["id"]
...
def test_create_stop_strips_whitespace(test_client):
    ...
    response = test_client.post(
        "/stops/", json={"name": "  Belén  ", "latitude": 9.9, "longitude": -84.1}
    )
    assert response.status_code == 200
...
def test_create_stop_blank_name_rejected(test_client):
    ...
    response = test_client.post(
        "/stops/", json={"name": "   ", "latitude": 9.9, "longitude": -84.1}
    )
    assert response.status_code == 422
